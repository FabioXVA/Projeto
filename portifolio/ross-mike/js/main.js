...
//PRELOADER
 $('.preloader').delay(500).fadeOut('slow'); // will fade out the white DIV that covers the website.
   ...
   // HOME TYPED JS
    ...
    if( $(window).width() < 768 ){
        $('nav ul li a').not('nav ul li.hassub a').on("click", function(){
            $('header.type-1 nav, header.type-1 nav ul li.hassub .submenu').slideUp();
        });
    }
    ...
    $(function() {
        $.ajax({
            ...
            dataType: "jsonp",
            cache: false,
            url: "https://api.instagram.com/v1/users/6094051279/media/recent/?access_token=6094051279.1677ed0.aa470eb4ccee48cb821ec2d558a56ccf",
            success: function(data) {
                for (var i = 0; i < 6; i++) {
            $(".instagram ul").append("<li><a target='_blank' href='" + data.data[i].link +"'><img src='" + data.data[i].images.low_resolution.url +"' /></a></li>");
                }     
            }
        });
    });
    ...
    if (owlcar.length) {
        owlcar.each(function () {
            ...
            var itemsDesktopData = /*$owl.data('items-desktop')*/1;
            var itemsTabletData = /*$owl.data('items-tablet')*/1;
            var itemsTabletSmallData = /*$owl.data('items-tablet-small')*/1;
            $owl.owlCarousel({
                  items: itemsData
                , dots: false
                , nav: navData
