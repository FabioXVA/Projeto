...
if(!( function_exists('ebor_framework_updates_notification') )){
  	function ebor_framework_updates_notification(){
  		...
  		if ( ! get_user_meta($user_id, 'foundry_notice') ) {
  			
			echo '
				<div class="notice notice-warning is-dismissible">
					<div class="content">
						<h3>Foundry 2.1.0 Has Made Some Big Changes - PLEASE READ</h3>
				        <p>Foundry was first released in 2015, since then we\'ve improved certain aspects of our themes to bring them inline with modern standards, this update of Foundry is part of that.</p>
				        <p>If you\'re a new user to Foundry, ignore this and continue setting up your theme with the documentation. Existing users:</p>
				        <ol>
				        	<li>Please follow the link below to update Visual Composer, Ebor Framework, and install "WP LESS".</li>
				        	<li>If your site is showing shortcodes instead of rendered content after step 1, please visit "appearance => themes" switch to another theme on this screen. After activating another theme, re-activate Foundry, Foundry will then run through its initial setup functions and resolve this for you.</li>
				        </ol>
...
if(!( function_exists('ebor_nag_ignore') )){
	function ebor_nag_ignore() {
	    ...
	    if ( isset($_GET['ebor_nag_ignore']) && '0' == $_GET['ebor_nag_ignore'] ) {
	    	add_user_meta($user_id, 'foundry_notice', 'true', true);
		}
	}
...
if(!( function_exists('ebor_icons_meta_field') )){
	function ebor_icons_meta_field( $field, $meta, $object_id, $object_type, $field_type_object ) {
		...
		echo '</div>';
		echo wp_specialchars_decode($field_type_object->input( array( 'type' => 'text' ) ));
		echo '</div>';
...
if(!( function_exists('ebor_breadcrumbs') )){ 
	function ebor_breadcrumbs() {
		...
		if( is_blog_page() && is_single() ){
			$breadcrumb .= '<li><a href="' . esc_url( get_permalink( get_option( 'page_for_posts' ) ) ) . '">' . get_option('blog_title','Our Blog') . '</a></li><li class="active">' . get_the_title( $post->ID ) . '</li>';
		} elseif( is_blog_page() ){
			$breadcrumb .= '<li class="active">' . get_option('blog_title','Our Blog') . '</li>';
...
if(!( function_exists('ebor_register_required_plugins') )){
	function ebor_register_required_plugins() {
		$plugins = array(
			...
			array(
			    'name'      => esc_html__('WP Less', 'foundry'),
			    'slug'      => 'wp-less',
			    'required'  => false,
			    'version' 	=> '1.0.0'
			),
			array(
			    'name'      => esc_html__('Custom Metaboxes 2', 'foundry'),
			    'slug'      => 'cmb2',
			    'required'  => true,
			    'version' 	=> '1.0.0'
			),
